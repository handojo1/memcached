...
static void process_config_command(conn *c) {
    out_string(c, "1\nmemcached-server|192.168.201.6|11211\n");
}
...
static void process_command(conn *c, char *command) {
    ...
    if (first == 'm' && tokens[COMMAND_TOKEN].length == 2) {
        ...
        } else if (strcmp(tokens[COMMAND_TOKEN].value, "config") == 0) {
            process_config_command(c);
        } else {
            out_string(c, "ERROR");
        }
    } else if (first == 'i') {
